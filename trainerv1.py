...
class Trainer:
    ...
    def compute_losses(self, inputs, outputs):
        ...
        sys_frame = self.d_fusion(inputs[("color_aug", -1, 0)], inputs[("color_aug", 0, 0)], inputs[("color_aug", 1, 0)], f_depth[("disp", 0)], s_depth[("disp", 0)], t_depth[("disp", 0)])
        target_depth_feature_t = self.d_encoder(sys_frame)
        target_depth_t = self.d_decoder(target_depth_feature_t)
        for scale in self.opt.scales:
            #scales=[0,1,2,3]
            loss = 0
            reprojection_losses = []
            ...
            loss += torch.abs(outputs[("disp", scale)] - target_depth_t[("disp", scale)]).mean()
            if self.opt.v1_multiscale:
                source_scale = scale
            else:
                source_scale = 0
