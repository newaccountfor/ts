...
class Fusion(nn.Module):
    ...
    def forward(self, f_f, s_f, t_f, d_f, d_s, d_t):
        f_f_f = torch.cat((f_f, d_f), 1)
        s_f_f = torch.cat((s_f, d_s), 1)
        t_f_f = torch.cat((t_f, d_t), 1)
        all_f = torch.cat((f_f_f, s_f_f, t_f_f), 1)
